...
class _SafeZoneHomeState extends State<SafeZoneHome> {
  ...
  Future<void> _locate() async {
    ...
    if (permission == LocationPermission.denied ||
      ...
      _message('Locatietoegang is nodig voor de kaart.');
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed: () => _message('Je hebt geen nieuwe meldingen.'),
            icon: Badge(
      ...
      body: [_overview(), _mapPage(), _community(), _contactsPage()][_page],
        ...
        Card(
          ...
          child: SwitchListTile(
            secondary: CircleAvatar(
              backgroundColor:
                  _tracking ? Colors.green : Colors.deepPurple.shade100,
              child: Icon(_tracking ? Icons.gps_fixed : Icons.gps_not_fixed),
            ),
            title: Text(_tracking ? 'Tracking actief' : 'Tracking uit',
                style: const TextStyle(fontWeight: FontWeight.bold)),
            subtitle: Text(_tracking
                ? 'Je locatie wordt gevolgd.'
                : 'Schakel tracking in voor locatie-waarschuwingen.'),
            value: _tracking,
            onChanged: (value) {
  ...
  Widget _mapPage() {
    return ListView(
      padding: const EdgeInsets.fromLTRB(16, 12, 16, 100),
      children: [
        Text('Veiligheidskaart',
            style: Theme.of(context)
                .textTheme
                .headlineSmall
                ?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 6),
        const Text('Bekijk echte kaartdata en je actuele positie.'),
        const SizedBox(height: 18),
        _map(),
        const SizedBox(height: 18),
        const Text('Onveilige zones',
  ...
  Widget _map() {
    final markers = _zones
        .map((zone) => Marker(
            point: zone.location,
            child: _Marker(color: zone.color, label: zone.name)))
        .toList();
    if (_position != null) {
      markers.add(Marker(
  ...
  Widget _community() {
    const reports = [
      'Slechte verlichting in de tunnel',
      'Toezicht verhoogd bij Station Zuid',
      'Veilige koffiebar open tot laat',
    ];
    return ListView(
      padding: const EdgeInsets.fromLTRB(16, 12, 16, 100),
      children: [
        ...
        const Text('Delen van veilige of onveilige routes helpt iedereen.'),
        const SizedBox(height: 16),
        FilledButton.icon(
          onPressed: () =>
              _message('Je melding is gedeeld met de community.'),
        ...
        ...reports.map((report) => Card(
              child: ListTile(
                ...
                title: Text(report),
                subtitle: const Text('Vandaag gemeld'),
              ),
            )),
      ],
    );
  ...
  Widget _contactsPage() {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        const Text(
            'Deze personen ontvangen een melding als je hulp nodig hebt.'),
        const SizedBox(height: 16),
        ..._trustedContacts.map((contact) => Card(
              child: ListTile(
                ...
                trailing: IconButton(
                  onPressed: () =>
                      _message('Bellen naar ${contact.name}...'),
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                CircleAvatar(
                  backgroundColor: zone.color.withValues(alpha: .15),
            ...
            Text(_risk > 70
                ? 'Blijf alert en verlaat het gebied als dat mogelijk is.'
                : 'Houd je route in de gaten.'),
            Align(
              alignment: Alignment.centerRight,
              child: Text(
                  'Tijd in zone: ${_seconds ~/ 60} min ${_seconds % 60} s'),
            ),
          ],
        ),
      ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Heb je direct hulp nodig?',
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  onPressed: () {
                    Navigator.pop(context);
                    _message('Stille melding verzonden naar je contacten.');
                  },
                  icon: const Icon(Icons.notifications_none),
                  label: const Text('Stuur een stille melding'),
                ),
              ),
            ],
          ),
...
class _Zone {
  ...
  const _Zone(this.name, this.district, this.risk, this.color, this.icon,
      this.location, this.minutes);
}
